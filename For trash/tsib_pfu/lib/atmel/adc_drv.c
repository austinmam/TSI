...
U16 adc_single_conversion(U8 adc_input_pin)
{
    ...
    // Enable ADC I/O-pin
	//removing this because it apparently is not a real register
    //ADC_PORT_OUT &= ~(1<<adc_input_pin);
	
	//edited so to work with AT90CAN
	ADMUX &= (0b11100000 | (adc_input_pin));
	
    //removing this because it apparently is not a real register
	//ADC_PORT_DIR &= ~(1<<adc_input_pin);
	
	//changed this to be the correct register name
    DIDR0  |=  (1<<adc_input_pin);        
